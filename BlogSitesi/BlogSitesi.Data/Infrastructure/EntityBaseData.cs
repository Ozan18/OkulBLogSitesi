...
namespace BlogSitesi.Data.Infrastructure
{
    public class EntityBaseData<T> : IData<T> where T : ModelBase
    {
        ...
        {
            return DeleteByKey(id, _context);
        }

        public DataResult DeleteByKey(int id, DbContext _context)
        {
            try
            {
       ...
       public DataResult DeleteBulk(Expression<Func<T, bool>> predicate)
        ...
        List<T> IData<T>.GetBy(Expression<Func<T, bool>> predicate, int pageNumber, int pageCount, string orderBy, bool isDesc)
        {
            try
            {
                return isDesc
